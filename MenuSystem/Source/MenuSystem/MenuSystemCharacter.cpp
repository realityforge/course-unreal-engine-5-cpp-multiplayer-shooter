...
void AMenuSystemCharacter::CreateGameSession() {
  if (OnlineSessionInterface.IsValid()) {
    ...
    if (OnlineSessionInterface->GetNamedSession(NAME_GameSession)) {
      ...
      if (!OnlineSessionInterface->DestroySession(NAME_GameSession)) {
        UE_LOG(LogTemp, Error,
               TEXT("AMenuSystemCharacter: Failed to DestroySession.\n"));
      }
    }
    ...
    UE_LOG(LogTemp, Error,
           TEXT("AMenuSystemCharacter: Calling CreateSession.\n"));
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName,
                                                   bool bWasSuccessful) {
  UE_LOG(LogTemp, Error,
         TEXT("AMenuSystemCharacter: OnCreateSessionComplete(%d).\n"),
         bWasSuccessful);
  if (bWasSuccessful) {
    if (GEngine) {
      GEngine->AddOnScreenDebugMessage(
...
void AMenuSystemCharacter::SetupPlayerInputComponent(
  ...
  if (UEnhancedInputComponent *EnhancedInputComponent =
    ...
    EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered,
                                       this, &AMenuSystemCharacter::Move);

    // Looking
    EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered,
                                       this, &AMenuSystemCharacter::Look);
  }
}
